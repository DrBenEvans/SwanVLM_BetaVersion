...
[RefPoint, RootProfile, RootChord, TipProfile, TipChord, Span, Sweep, Dihedral, GeoProps, RootIncidence, TipIncidence, Mirrored, Inverted, SurfaceArea,SectionArea] = ExcelConfigRead(FileName);
...
geo.SurfaceRefArea=sum(SurfaceArea(2:end,:),2);
geo.WingRefArea=sum(SurfaceArea(1,:));
b0=TipChord(1,1:-1+length(TipChord(1,:))); % root chords of wing sections
btip=TipChord(1,2:length(TipChord(1,:)));  % tip chords of wing sections

%Calculate Mean Aerodynamic Chord of a whole wing considering all wing's
%sections 
geo.WingAeroChords(1)=2/3*(RootChord(1)+TipChord(1)-RootChord(1)*TipChord(1)/(RootChord(1)+TipChord(1)));
geo.WingAeroChords(2:length(TipChord(1,:) )) = 2/3*(b0+btip-b0.*btip./(b0+btip));
geo.WingMeanAeroChord=sum((geo.WingAeroChords.*geo.SurfaceArea(1,:)))/(geo.WingRefArea);
if length(TipChord(1,:))>1
b0_tail=TipChord(2:end,1:-1+length(TipChord(2,:)));
btip_tail=TipChord(2:end,2:length(TipChord(2,:)));
geo.TailAeroChords(:,1)=2/3*(RootChord(2:end)+TipChord(2:end,1)-RootChord(2:end).*TipChord(2:end,1)./(RootChord(2:end)+TipChord(2:end,1)));
geo.TailAeroChords(:,2:length(TipChord(1,:) )) = 2/3*(b0_tail+btip_tail-b0_tail.*btip_tail./(b0_tail+btip_tail));
geo.TailMeanAeroChords=sum(geo.TailAeroChords.*geo.SurfaceArea(2:end,:),2,'omitnan')./geo.SurfaceRefArea;
geo.MeanAeroChords = [geo.WingMeanAeroChord geo.TailMeanAeroChords'];
geo.SurfaceAreas = [geo.WingRefArea geo.SurfaceRefArea'];
else 
geo.MeanAeroChords = [geo.WingMeanAeroChord];
geo.SurfaceAreas = [geo.WingRefArea];
end
...
function [RefPoint, RootProfile, RootChord, TipProfile, TipChord, SectionSpan, SectionSweep, SectionDihedral, GeoProps, RootIncidence, TipIncidence, Mirrored, Inverted, SurfaceArea,SectionArea] = ExcelConfigRead(FileName)
...
for i = 1:A
    ...
    SurfaceArea(num(i,1),num(i,2)) = num(i,17);
    ...
    SectionArea(num(i,1),num(i,2))=SectionSpan(num(i,1),num(i,2))*(RootChord(num(i,1),1)+TipChord(num(i,1),num(i,2)))/2;
    end
    
    if num(i,2) > 1
    SectionArea(num(i,1),num(i,2))=SectionSpan(num(i,1),num(i,2))*(RootChord(num(i-1,1),1)+TipChord(num(i,1),num(i,2)))/2;
    end
...
switch Profile
    ...
    otherwise
        ...
        LowerSurfaceFit = interp1(chord*LowerSurface(:,1),LowerSurface(:,2),'cubic','pp');
        UpperSurfaceFit = interp1(chord*UpperSurface(:,1),UpperSurface(:,2),'cubic','pp');
